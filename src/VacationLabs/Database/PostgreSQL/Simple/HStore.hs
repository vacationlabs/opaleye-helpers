...
module VacationLabs.Database.PostgreSQL.Simple.HStore where
...
parseHStore = do
    ...
    return $ ((HStoreList).catMaybes) <$> sequence kvs

parseHStoreKeyVal :: P.Parser (Either UnicodeException (Maybe (Text,Text)))
parseHStoreKeyVal = do
  ...
  case mkey of
    ...
    Right key -> do
      ...
      _ <- P.string "=>"
      skipWhiteSpace
      nval <- P.eitherP parseHStoreNull parseHStoreText 
      case nval of
        Right mval -> case mval of
          Left  err -> return $ (Left err)
          Right val -> return $ (Right $ Just (key,val))
        Left _ -> return $ Right Nothing

skipWhiteSpace :: P.Parser ()
skipWhiteSpace = P.skipWhile P.isSpace_w8

parseHStoreNull :: P.Parser (Either UnicodeException Text)
parseHStoreNull = TS.decodeUtf8' <$> P.string "NULL"
